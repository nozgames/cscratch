...
static Props LoadMetaFile(const fs::path& source_path);
...
static Props LoadMetaFile(const fs::path& source_path)
{
    ...
    if (!fs::exists(meta_path))
        throw std::runtime_error("Meta file does not exist: " + meta_path.string());
    
    // Load the meta file using Stream
    Stream* stream = LoadStream(nullptr, meta_path);
    if (!stream)
        throw std::runtime_error("Failed to load meta file: " + meta_path.string());
    
    Props* props = Props::Load(stream);
    Destroy(stream);
    
    if (!props)
        throw std::runtime_error("Failed to parse meta file: " + meta_path.string());
    
    Props result = *props;  // Copy the props
    delete props;           // Clean up the allocated Props

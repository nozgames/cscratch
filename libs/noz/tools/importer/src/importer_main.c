...
import_job_t* to_import_job_impl(const void* job) { return (import_job_t*)to_object((const object_t*)job, type_unknown); }
...
int main(int argc, char* argv[])
{
	...
	file_watcher_shutdown();
	object_free(g_config);
	object_free(g_import_queue);
	return 0;
}

static bool import_job_path_predicate(const object_t* o, void* data)
{
...
void process_file_change(const path_t* file_path, file_change_type_t change_type, asset_importer_traits_t** importers)
{
	...
	import_job_t* new_job = (import_job_t*)object_alloc(NULL, sizeof(import_job_t), type_unknown);
	if (!new_job)
		return;

	new_job->source_path = source_path;
	new_job->importer = selected_importer;
	list_add(g_import_queue, (object_t*)new_job);
}
	...
	while (made_progress && !list_empty(g_import_queue))
	{
		...
		for (size_t i = 0; i < list_count(g_import_queue); i++)
		{
			...
			else
			{
				...
				list_add(remaining_jobs, (object_t*)job);
			}
		}

...
GMATH_INLINE vec4_t vec4_add(vec4_t left, vec4_t right)
{
    vec4_t result;
#ifdef GMATH_USE_SSE
    _mm_store_ps((float*)&result, _mm_add_ps(_mm_load_ps((float*)&left), _mm_load_ps((float*)&right)));
#else
    result.x = left.x + right.x;
    result.y = left.y + right.y;
    result.z = left.z + right.z;
...
GMATH_INLINE vec4_t vec4_mul(vec4_t left, vec4_t right)
{
    vec4_t result;
#ifdef GMATH_USE_SSE
    _mm_store_ps((float*)&result, _mm_mul_ps(_mm_load_ps((float*)&left), _mm_load_ps((float*)&right)));
#else
    result.x = left.x * right.x;
    result.y = left.y * right.y;
    result.z = left.z * right.z;
...
GMATH_INLINE mat4_t mat4_transpose(mat4_t mat)
{
    mat4_t result = mat;
#ifdef GMATH_USE_SSE
    _MM_TRANSPOSE4_PS(result.sse_cols[0], result.sse_cols[1], result.sse_cols[2], result.sse_cols[3]);
#else
...
GMATH_INLINE mat4_t mat4_mul(mat4_t left, mat4_t right)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        __m128 row = _mm_add_ps(_mm_add_ps(_mm_mul_ps(brod1, right.sse_cols[0]), _mm_mul_ps(brod2, right.sse_cols[1])),
...
GMATH_INLINE vec4_t mat4_mulv(mat4_t left, vec4_t right)
{
    ...
    result.v[0] = left.cols[0][0] * right.v[0] + left.cols[1][0] * right.v[1] + left.cols[2][0] * right.v[2] +
                  left.cols[3][0] * right.v[3];
    result.v[1] = left.cols[0][1] * right.v[0] + left.cols[1][1] * right.v[1] + left.cols[2][1] * right.v[2] +
                  left.cols[3][1] * right.v[3];
    result.v[2] = left.cols[0][2] * right.v[0] + left.cols[1][2] * right.v[1] + left.cols[2][2] * right.v[2] +
                  left.cols[3][2] * right.v[3];
    result.v[3] = left.cols[0][3] * right.v[0] + left.cols[1][3] * right.v[1] + left.cols[2][3] * right.v[2] +
...
GMATH_INLINE mat4_t mat4_inverse(mat4_t mat)
{
    ...
    float t[6];
    float a = mat.cols[0][0], b = mat.cols[0][1], c = mat.cols[0][2], d = mat.cols[0][3], e = mat.cols[1][0],
          f = mat.cols[1][1], g = mat.cols[1][2], h = mat.cols[1][3], i = mat.cols[2][0], j = mat.cols[2][1],
          k = mat.cols[2][2], l = mat.cols[2][3], m = mat.cols[3][0], n = mat.cols[3][1], o = mat.cols[3][2],
          p = mat.cols[3][3];
...
GMATH_INLINE mat4_t mat4_perspective(float fovy, float aspect_ratio, float znear, float zfar)
{
...
GMATH_INLINE mat4_t mat4_rotate(mat4_t mat, float angle, vec3_t axis)
{
    ...
    mat4_t result;
    result.v[0] = vec4_add(vec4_add(vec4_muls(mat.v[0], rotate.cols[0][0]), vec4_muls(mat.v[1], rotate.cols[0][1])),
                           vec4_muls(mat.v[2], rotate.cols[0][2]));
    result.v[1] = vec4_add(vec4_add(vec4_muls(mat.v[0], rotate.cols[1][0]), vec4_muls(mat.v[1], rotate.cols[1][1])),
                           vec4_muls(mat.v[2], rotate.cols[1][2]));
    result.v[2] = vec4_add(vec4_add(vec4_muls(mat.v[0], rotate.cols[2][0]), vec4_muls(mat.v[1], rotate.cols[2][1])),
...
GMATH_INLINE quat_t quat_normalize(quat_t left)
{
    ...
    return (quat_t){left.x * one_over_length, left.y * one_over_length, left.z * one_over_length,
...
GMATH_INLINE float remap(float n, float start1, float stop1, float start2, float stop2)
{
